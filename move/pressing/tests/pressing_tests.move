...
fun press_mints_sequential_records_derived_off_the_pressing() {
    ...
    let mut p = pressing::new_for_testing<SUI>(
        release, 3, pressing::new_fixed_price(0), 0, option::none(), &mut ctx,
    );

    let r1 = pressing::press(&mut p, coin::zero<SUI>(&mut ctx), &cfg, &clk, &mut ctx);
    let r2 = pressing::press(&mut p, coin::zero<SUI>(&mut ctx), &cfg, &clk, &mut ctx);
...
fun press_records_amount_paid() {
    ...
    let mut p = pressing::new_for_testing<SUI>(
        id(@0xBEEF), 0, pressing::new_floor_price(5), 0, option::none(), &mut ctx,
    );
    let payment = coin::mint_for_testing<SUI>(8, &mut ctx);
    let r = pressing::press(&mut p, payment, &cfg, &clk, &mut ctx);
...
fun is_live_reflects_window() {
    ...
    let p = pressing::new_for_testing<SUI>(
        id(@0xBEEF), 0, pressing::new_fixed_price(0), 100, option::some(200), &mut ctx,
    );
...
fun press_aborts_when_witness_not_authorized() {
    ...
    let mut p = pressing::new_for_testing<SUI>(
        id(@0xBEEF), 0, pressing::new_fixed_price(0), 0, option::none(), &mut ctx,
    );
    let r = pressing::press(&mut p, coin::zero<SUI>(&mut ctx), &cfg, &clk, &mut ctx);
...
fun press_aborts_on_underpayment() {
    ...
    let mut p = pressing::new_for_testing<SUI>(
        id(@0xBEEF), 0, pressing::new_fixed_price(100), 0, option::none(), &mut ctx,
    );
...
fun press_aborts_before_open() {
    ...
    let mut p = pressing::new_for_testing<SUI>(
        id(@0xBEEF), 0, pressing::new_fixed_price(0), 100, option::none(), &mut ctx,
    );
    let r = pressing::press(&mut p, coin::zero<SUI>(&mut ctx), &cfg, &clk, &mut ctx);
...
fun press_aborts_after_close() {
    ...
    let mut p = pressing::new_for_testing<SUI>(
        id(@0xBEEF), 0, pressing::new_fixed_price(0), 0, option::some(100), &mut ctx,
    );
    let r = pressing::press(&mut p, coin::zero<SUI>(&mut ctx), &cfg, &clk, &mut ctx);
...
fun editions_created_in_sequence() {
    ...
    pressing::new<SUI>(
        &mut registry, &rel, &cap, pressing::new_fixed_price(0), 0, 0, option::none(), &clk,
    );
    pressing::new<SUI>(
        &mut registry, &rel, &cap, pressing::new_fixed_price(0), 1, 0, option::none(), &clk,
    );
...
fun invalid_window_aborts() {
    ...
    pressing::new<SUI>(
        &mut registry, &rel, &cap, pressing::new_fixed_price(0), 0, 100, option::some(50), &clk,
    );
